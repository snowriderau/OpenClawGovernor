...
6. When changing notification channel config, always verify the FULL message path:
   ...
   - Channel config (e.g., `channels.telegram`) — groupPolicy, allowFrom, dmPolicy
...
9. Delegation model: the primary orchestrator agent is the only channel-facing agent. All other agents report to it via `sessions_spawn agent:"main"`. The orchestrator decides what reaches the user.
